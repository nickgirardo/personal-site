...
export const SATPost = ():ReactElement => {
  return (
    <Page sidebar={ DefaultSidebar }>
      ...
      <p>
        Like other constraint solvers, SAT Solvers can enable dynamic programming.  Once a programmer describes their problem (in a form the solver can understand) the work of actually working through a solution is offloaded to the solver.  Dynamic programming's inversion of work from computing a solution to describing a problem can be incredibly liberating.  Some forms of dynamic programming can be found everywhere in computing.  SAT Solvers are specifically interesting to me because of how low level they are.  As will be demonstrated, SAT Solvers can answer complex questions which are built of incredibly simple primitives.
      </p>
      <p>
      ...
      <p>
        Once we have our formula in CNF, setting the SAT Solver up is easy.  Here is how the third of the above three formulas would be set up:
      </p>
      <CodeRegion
        code={ code4 }
        codeHeight='16em'
      ...
      />
      <p>
        Note that above each clause is represented by one of the sub-arrays to the main clause array.  Each literal is represented by an integer: negative if the literal is negated and positive otherwise.
      </p>
      ...
      />
      <p>
        Now (much like our clause input) our solution output has positive integers representing literals set to true and negative integers representing literals set to false.  These form are equivalent, if you so desired you could write a function <code>clauseAsSolution</code> to reverse the above process.  I find the above form more easy to reason about, and it will prove useful in the next section&hellip;
      </p>
      <h3>Multiple satisfing solutions</h3>
      <p>
      ...
      />
      <p>
        This method&hellip; isn't great. For one, while getting all of the unique solutions you will almost certainly get a bunch of duplicates.  Perhaps more importantly, it is impossible to tell when all unique solutions have been found.
      </p>
      <p>
      ...
      <h3>A practical example: Sudoku</h3>
      { /* TODO remove */ }
